...
# xx/xx
# xx-xx
dateReg = '([\s](1[0-2])[-]([1-9]|1[0-9]|2[0-9]|3[0-1]))[-](\d{2}|19\d{2}|20[01][0-9][\s])|' \
              '([\s]([1-9]|1[0-2])[/]([1-9]|1[0-9]|2[0-9]|3[0-1]))[/](\d{2}|19\d{2}|20[01][0-9][\s])|' \
              '([\s]([1-9]|1[0-2])[-]([1-9]|1[0-9]|2[0-9]|3[0-1])[\s])|' \
              '([\s]([1-9]|1[0-2])[/]([1-9]|1[0-9]|2[0-9]|3[0-1])[\s])'

date_reg = re.compile(dateReg)
...
def check_for_date(patient, note, chunk, output_handle):
    ...
    for match in date_reg.finditer(chunk):
        ...
        # result = str(match.start()) + ' ' + str(match.start()) + ' ' + str(match.end())
